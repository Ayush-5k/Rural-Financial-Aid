...
const BudgetCalendar = ({ bills }) => {
	...
	const filteredBills = bills.filter((bill) => {
		...
		return (
			(!selectedRange.start ||
				dueDate.isSameOrAfter(selectedRange.start, "day")) &&
		...
		<div className="bg-white p-6 rounded-lg shadow">
			...
			<div className="mt-4">
				<h3 className="font-semibold mb-2">
					Bills Due{" "}
					{selectedRange.start
						? `(${moment(selectedRange.start).format("MMM D")} - ${moment(
								selectedRange.end
						  ).format("MMM D")})`
						: ""}
				</h3>
				<div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
					{filteredBills.map((bill) => (
						<div key={bill.id} className="bg-gray-200 p-2 rounded">
							<p className="text-sm">{bill.name}</p>
							<p className="font-semibold">Rs {bill.amount}</p>
							<p className="text-xs text-gray-600">
								{moment(bill.dueDate).format("MMM D, YYYY")}
							</p>
						</div>
					))}
				</div>
