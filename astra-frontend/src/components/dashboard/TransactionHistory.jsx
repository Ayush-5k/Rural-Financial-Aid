...
const API_URL = "https://AstraPlan-backend.azurewebsites.net";
	...
	useEffect(() => {
		const fetchTransactions = async () => {
			try {
				...
				const normalizedTransactions = response.data.transactions.map(
					(transaction) => ({
					...
					<tbody>
						{transactions.map((transaction, index) => (
							<tr key={index} className="border-t">
								...
								<td
									className={`px-4 py-2 ${getAmountColor(transaction.Amount)}`}
								>
									₹ {transaction.Amount.toFixed(2)}{" "}
									{/* Ensure amount is formatted as a number */}
								</td>
